...
var forEachReverse = function(array, fn){
...
System.prototype.mergeClusters = function(clusterA, clusterB){
  debugger;
  var index = this.getClusterIndex(clusterB.id);
  this.clusters.splice(index, 1);
  var atomsB = clusterB.atoms;
  var atomsA = clusterA.atoms;
  forEach(atomsA, function(atom){
    forEachReverse(atom.connections, function(edge, i){
      if(edge.id === clusterB.id){
  ...
  forEach(atomsB, function(atom){
    forEachReverse(atom.connections, function(edge, i){
      if(edge.id === clusterA.id){
        atom.connections.splice(i, 1);
      }
    })
    clusterA.addAtom(atom);
  })
  forEach(this.clusters, function(cluster){
    forEach(cluster.atoms, function(atom){
      forEach(atom.connections, function(edge){
        if(edge.id === clusterB.id){
          edge.id = clusterA.id;
        }
      })
